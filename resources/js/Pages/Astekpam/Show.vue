...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex flex-col sm:flex-row gap-3 sm:items-center justify-between font-sans print:hidden">
                <Link :href="route('astekpam.index')">
                    <Button variant="outline" size="sm" class="rounded-full w-full sm:w-auto">
                        <ChevronLeft class="w-4 h-4 mr-1" /> Kembali
                    </Button>
                </Link>
                <div class="flex flex-wrap gap-2">
                    <Link v-if="$page.props.auth.user.roles?.includes('admin')" :href="route('astekpam.edit', astekpam.id)" class="flex-1 sm:flex-none">
                        <Button class="bg-amber-500 hover:bg-amber-600 font-sans shadow-sm text-white w-full">
                            <Edit class="w-4 h-4 mr-2" /> Edit
                        </Button>
                    </Link>

                    <Button @click="copyToClipboard" :variant="isCopied ? 'default' : 'outline'" class="font-sans transition-all flex-1 sm:flex-none">
                        <component :is="isCopied ? Check : Copy" class="w-4 h-4 mr-2" />
                        {{ isCopied ? 'Tersalin' : 'Salin' }}
                    </Button>
                    
                    <Button @click="printPage" class="bg-zinc-800 hover:bg-zinc-900 font-sans shadow-md text-white flex-1 sm:flex-none">
        ...
        <!-- Container luar: Hilangkan padding horizontal pada mobile (px-0), beri px-4 hanya untuk layar tablet ke atas -->
        <div class="max-w-4xl mx-auto py-6 sm:py-10 px-0 sm:px-4 font-sans print:p-0 print:max-w-full">
            
            <!-- Card inti: Penuh tanpa border radius di mobile, baru melengkung dan berjarak lega di layar desktop -->
            <Card class="border-none shadow-xl sm:shadow-2xl bg-white p-5 sm:p-8 md:p-16 rounded-none sm:rounded-xl text-zinc-900 font-sans text-[13px] sm:text-[15px] leading-relaxed print:shadow-none print:p-0 break-words">
                
                <!-- TAMPILAN FOTO LAPORAN -->
                <div v-if="astekpam.foto_laporan" class="mb-6 sm:mb-8 flex flex-col items-center bg-zinc-50/80 p-3 sm:p-4 rounded-xl border border-zinc-200 print:hidden">
                    <span class="text-[11px] sm:text-xs font-bold text-zinc-500 mb-3 w-full text-center uppercase tracking-wider flex items-center justify-center gap-2">
                ...
                <!-- GRID PROPORSIONAL: Kolom teks (135px di HP, 160px di Laptop), Kolom titik dua (10px - 15px), Kolom data (Sisa ruang). Items-start menjaga kelurusan ke atas -->
                <div class="grid grid-cols-[135px_10px_1fr] sm:grid-cols-[160px_15px_1fr] gap-y-1.5 sm:gap-y-1 items-start font-sans">
                    <div>Hari/Tgl</div><div>:</div><div class="font-semibold">{{ displayHari }}</div>
                    <div>Pukul</div><div>:</div><div class="font-semibold">{{ astekpam.pukul }} WIB</div>
                </div>
                <br>
                ...
                <p class="font-bold mt-3 mb-1">A. JUMLAH PENGHUNI</p>
                <div class="grid grid-cols-[135px_10px_1fr] sm:grid-cols-[160px_15px_1fr] gap-y-1.5 sm:gap-y-1 items-start font-sans">
                    ...
                    <div class="pl-3 sm:pl-5">* Blok A</div><div>:</div><div>{{ formatOrg(astekpam.blok_a) }}</div>
                    <div class="pl-3 sm:pl-5">* Blok B</div><div>:</div><div>{{ formatOrg(astekpam.blok_b) }}</div>
                    <div class="pl-3 sm:pl-5">* Dapur</div><div>:</div><div>{{ formatOrg(astekpam.dapur) }}</div>
                    <div class="pl-3 sm:pl-5">* Klinik</div><div>:</div><div>{{ formatOrg(astekpam.klinik) }}</div>
                    
                    <div>Jumlah</div><div>:</div><div></div>
                    <div class="pl-3 sm:pl-5">* Didalam Lapas</div><div>:</div><div>{{ formatOrg(astekpam.dalam_lapas) }}</div>
                    <div class="pl-3 sm:pl-5">* Diluar Lapas</div><div>:</div><div>{{ formatOrg(astekpam.luar_lapas) }}</div>
                    
                    <div class="mt-2 sm:mt-1 pr-2">4. Keterangan di luar Lapas</div><div class="mt-2 sm:mt-1">:</div><div class="mt-2 sm:mt-1"></div>
                    <div class="pl-3 sm:pl-5">* Rawat Inap RS</div><div>:</div><div>{{ formatItems(astekpam.rawat_inap_items) }}</div>
                    <div class="pl-3 sm:pl-5">* Berobat RS</div><div>:</div><div>{{ formatItems(astekpam.berobat_items) }}</div>
                    <div class="pl-3 sm:pl-5 pr-2">* Lain-lain (bon luar)</div><div>:</div><div>{{ formatItems(astekpam.bon_luar_items) }}</div>
                    
                    <div class="font-bold mt-2 sm:mt-1">5. Total Jumlah WBP</div><div class="mt-2 sm:mt-1">:</div><div class="font-bold mt-2 sm:mt-1">{{ formatOrg(astekpam.total_wbp) }}</div>
                </div>
                <br>

                <p class="font-bold mb-1">B. PERSONIL PENGAMANAN</p>
                <div class="grid grid-cols-[135px_10px_1fr] sm:grid-cols-[160px_15px_1fr] gap-y-1.5 sm:gap-y-1 items-start font-sans">
                    <div class="font-semibold">1. {{ astekpam.rupam_pilihan }}</div><div></div><div></div>
                    <div class="pl-3 sm:pl-5">Jumlah</div><div>:</div><div>{{ astekpam.rupam_jumlah ? astekpam.rupam_jumlah + ' Org' : '-' }}</div>
                    <div class="pl-3 sm:pl-5">Hadir</div><div>:</div><div>{{ astekpam.rupam_hadir ? astekpam.rupam_hadir + ' Org' : '-' }}</div>
                    <div class="pl-3 sm:pl-5">Tidak Hadir</div><div>:</div><div>{{ astekpam.rupam_tidak_hadir ? astekpam.rupam_tidak_hadir + ' Org' : '-' }}</div>
                    <div class="pl-3 sm:pl-5">Keterangan</div><div>:</div><div>{{ astekpam.rupam_keterangan || '-' }}</div>
                    
                    <div class="font-semibold mt-2 sm:mt-1 pr-1">2. SATGAS P2U ({{ astekpam.p2u_rupam || '-' }})</div><div class="mt-2 sm:mt-1"></div><div class="mt-2 sm:mt-1"></div>
                    <div class="pl-3 sm:pl-5">Jumlah</div><div>:</div><div>{{ astekpam.p2u_jumlah ? astekpam.p2u_jumlah + ' Org' : '-' }}</div>
                    <div class="pl-3 sm:pl-5">Hadir</div><div>:</div><div>{{ astekpam.p2u_hadir ? astekpam.p2u_hadir + ' Org' : '-' }}</div>
                    <div class="pl-3 sm:pl-5">Keterangan</div><div>:</div><div>{{ astekpam.p2u_keterangan || '-' }}</div>
                </div>
                <br>

                <p class="font-bold mb-1">3. Pembagian Tugas :</p>
                <div class="grid grid-cols-[135px_10px_1fr] sm:grid-cols-[160px_15px_1fr] gap-y-1.5 sm:gap-y-1 items-start font-sans">
                    <div class="font-semibold">a. Ka. Rupam</div><div>:</div><div>{{ astekpam.tugas.ka_rupam || '-' }}</div>
                    
                    <div class="pl-3 sm:pl-5">Wakarupam</div><div>:</div><div>{{ astekpam.tugas.wakarupam || '-' }}</div>
                    
                    <div class="font-semibold mt-2 sm:mt-1">b. Petugas P2U</div><div class="mt-2 sm:mt-1">:</div><div class="mt-2 sm:mt-1"></div>
                    <div class="pl-3 sm:pl-5">Kasatgas</div><div>:</div><div>{{ astekpam.tugas.kasatgas_p2u || '-' }}</div>
                    <div class="pl-3 sm:pl-5">Wakasatgas</div><div>:</div><div>{{ astekpam.tugas.wakasatgas_p2u || '-' }}</div>
                    
                    <div class="font-semibold mt-2 sm:mt-1">c. Petugas Blok</div><div class="mt-2 sm:mt-1">:</div><div class="mt-2 sm:mt-1"></div>
                    <div class="pl-3 sm:pl-5">Blok A</div><div>:</div><div>{{ formatNamaTugas(astekpam.tugas.blok_a) }}</div>
                    <div class="pl-3 sm:pl-5">Blok B</div><div>:</div><div>{{ formatNamaTugas(astekpam.tugas.blok_b) }}</div>
                    
                    <div class="font-semibold mt-2 sm:mt-1 pr-1">d. Petugas Pos Atas</div><div class="mt-2 sm:mt-1">:</div><div class="mt-2 sm:mt-1"></div>
                    <div class="pl-3 sm:pl-5">* Menara 1</div><div>:</div><div>{{ formatNamaTugas(astekpam.tugas.menara_1) }}</div>
                    <div class="pl-3 sm:pl-5">* Menara 2</div><div>:</div><div>{{ formatNamaTugas(astekpam.tugas.menara_2) }}</div>
                    <div class="pl-3 sm:pl-5">* Menara 3</div><div>:</div><div>{{ formatNamaTugas(astekpam.tugas.menara_3) }}</div>
                    <div class="pl-3 sm:pl-5">* Menara 4</div><div>:</div><div>{{ formatNamaTugas(astekpam.tugas.menara_4) }}</div>
                    
                    <div class="mt-2 sm:mt-1">e. Petugas Jaga RS</div><div class="mt-2 sm:mt-1">:</div><div class="mt-2 sm:mt-1">{{ astekpam.tugas.jaga_rs || '-' }}</div>
                    ...
                    <div class="font-bold border-t border-slate-100 pt-1 mt-1">l. Petugas Laporan</div><div class="border-t border-slate-100 pt-1 mt-1">:</div><div class="font-bold border-t border-slate-100 pt-1 mt-1 uppercase">{{ astekpam.tugas.petugas_laporan || '-' }}</div>
                </div>
                <br>
