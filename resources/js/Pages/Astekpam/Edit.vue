...
const props = defineProps({
    astekpam: Object, // Data laporan yang sedang di-edit
    users: {
        type: Array,
        default: () => []
    },
...
// State untuk Preview Foto (Load dari data yang sudah ada jika ada)
const previewUrl = ref(props.astekpam?.foto_laporan ? `/storage/${props.astekpam.foto_laporan}` : null);
...
// ==========================================
// FUNGSI PARSING DATA JSON (ARRAY & OBJECT)
// ==========================================
const parseJsonArray = (data) => {
    if (!data) return [{ ket: '' }];
    if (typeof data === 'string') {
        try { data = JSON.parse(data); } catch(e) { return [{ ket: '' }]; }
    }
...
const parseJsonTugas = (data) => {
    const defaultTugas = {
        ...
        jaga_rs: '', banjaga: '', piket_dapur: '', amanah: '', petugas_laporan: ''
    };

    if (!data) return defaultTugas;
    
    let parsedData = data;
    if (typeof data === 'string') {
        try { parsedData = JSON.parse(data); } catch(e) { return defaultTugas; }
    }
    
    // Merge data existing dengan struktur default agar tidak ada property yang hilang/undefined
    return { ...defaultTugas, ...parsedData };
};
...
// ==========================================
const form = useForm({
    // PERHATIAN: Untuk update dengan file upload di Laravel, kita harus menggunakan POST dan _method: 'PUT'
    _method: 'put', 
    
    tanggal: props.astekpam.tanggal,
    pukul: props.astekpam.pukul,
    dari_rupam: props.astekpam.dari_rupam,
    dari_shift: props.astekpam.dari_shift,
    ke_rupam: props.astekpam.ke_rupam,
    ke_shift: props.astekpam.ke_shift,
    pimpinan: props.astekpam.pimpinan,
    
    // Field Foto Laporan (biarkan null jika tidak ingin mengganti foto lama)
    foto_laporan: null,
    
    kapasitas: props.astekpam.kapasitas,
    narapidana: props.astekpam.narapidana,
    blok_a: props.astekpam.blok_a,
    blok_b: props.astekpam.blok_b,
    dapur: props.astekpam.dapur,
    klinik: props.astekpam.klinik,
    dalam_lapas: props.astekpam.dalam_lapas,
    luar_lapas: props.astekpam.luar_lapas,
    
    rawat_inap_items: parseJsonArray(props.astekpam.rawat_inap_items),
    berobat_items: parseJsonArray(props.astekpam.berobat_items),
    bon_luar_items: parseJsonArray(props.astekpam.bon_luar_items),
    
    total_wbp: props.astekpam.total_wbp,

    rupam_pilihan: props.astekpam.rupam_pilihan,
    rupam_jumlah: props.astekpam.rupam_jumlah,
    rupam_hadir: props.astekpam.rupam_hadir,
    rupam_keterangan: props.astekpam.rupam_keterangan,
    p2u_jumlah: props.astekpam.p2u_jumlah,
    p2u_hadir: props.astekpam.p2u_hadir,
    p2u_keterangan: props.astekpam.p2u_keterangan,

    tugas: parseJsonTugas(props.astekpam.tugas)
});
...
const handleFileUpload = (e) => {
    ...
    if (file) {
        if (file.size > 10 * 1024 * 1024) {
            ...
            previewUrl.value = props.astekpam?.foto_laporan ? `/storage/${props.astekpam.foto_laporan}` : null;
            return;
        }
...
// ==========================================
// LOGIKA FILTER ANGGOTA REGU
// ==========================================
const loadAnggotaRegu = (rupam) => {
    if (!rupam || !props.users || props.users.length === 0) {
        anggotaReguOptions.value = [];
        return;
    }
    const bersihkanTeks = (teks) => String(teks || '').toLowerCase().replace(/[\s.]/g, '');
    const targetRegu = bersihkanTeks(rupam);

    anggotaReguOptions.value = props.users.filter(u => {
...
// Jalankan load dropdown anggota saat pertama kali komponen dirender
onMounted(() => {
    if (form.rupam_pilihan) {
        loadAnggotaRegu(form.rupam_pilihan);
    }
});

// Jika pilihan regu diubah OLEH USER, baru reset nilai tugasnya
watch(() => form.rupam_pilihan, (newRupam, oldRupam) => {
    // Hindari reset pada saat inisialisasi awal
    if (oldRupam !== undefined) {
        loadAnggotaRegu(newRupam);
        ...
        const cariJabatan = (kataKunci) => {
            const user = anggotaReguOptions.value.find(u => bersihkanTeks(u.jabatan).includes(kataKunci));
...
// ==========================================
const submitLaporan = () => {
    // Karena menggunakan PUT dengan FormData, route mengarah ke update
    form.post(route('admin.astekpam.update', props.astekpam.id), {
        forceFormData: true, 
        preserveScroll: true,
        onError: (errors) => {
...
<template>
    <Head title="Edit Laporan Astekpam" />
    <AuthenticatedLayout>
            ...
            <div class="max-w-4xl mx-auto w-full">
                ...
                <div class="mb-5 sm:mb-8 flex items-start">
                    <Link :href="route('astekpam.index')">
                        <Button variant="outline" size="sm" class="rounded-full bg-white border-zinc-200 text-zinc-600 hover:text-zinc-900 shadow-sm h-10 sm:h-9 px-4 sm:px-4 font-bold tracking-wide w-full sm:w-auto">
                            <ChevronLeft class="w-4 h-4 mr-1.5" /> Kembali ke Riwayat
                        </Button>
                    </Link>
                </div>
                ...
                <div class="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4 font-sans">
                    <div class="text-center sm:text-left">
                        <h1 class="text-2xl sm:text-3xl font-extrabold text-zinc-900 tracking-tight">Edit Laporan Astekpam</h1>
                ...
                <div v-if="Object.keys(form.errors).length > 0 && !isPreviewActive" class="mb-6 p-4 bg-red-50/80 border border-red-200 rounded-xl text-red-600 text-sm font-semibold">
                    <div class="flex items-center gap-2 mb-2">
                        <ShieldAlert class="w-5 h-5" /> Gagal Menyimpan. Ada field yang tidak valid:
                    </div>
                        ...
                        <CardContent class="p-5 sm:p-6">
                            <div class="flex flex-col md:flex-row items-start gap-6">
                                <div class="flex-1 w-full">
                                    <Label class="text-[11px] sm:text-xs font-bold text-zinc-500 tracking-wider mb-2 block">GANTI FOTO (MAKS. 10MB - Opsi)</Label>
                                    ...
                                    <div class="relative group mt-2">
                                        ...
                                        <div class="absolute inset-y-0 left-10 flex items-center pointer-events-none">
                                            <span v-if="!form.foto_laporan" class="text-sm text-zinc-400 font-medium">Biarkan kosong jika tidak ingin mengubah foto...</span>
                    ...
                    <Card class="rounded-2xl border border-zinc-200 shadow-sm overflow-hidden bg-white">
                        ...
                        <CardContent class="p-5 sm:p-6 space-y-5 sm:space-y-6">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div class="space-y-2">
                                    <Label class="text-[11px] sm:text-xs font-medium text-zinc-600">Kapasitas Lapas</Label>
                                    <Input type="number" v-model="form.kapasitas" class="h-12 sm:h-11 text-base sm:text-sm rounded-xl sm:rounded-lg"/>
                                </div>
                                <div class="space-y-2">
                                    <Label class="text-[11px] sm:text-xs font-bold text-indigo-700">Total Narapidana</Label>
                            ...
                            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
                                <div v-for="b in ['blok_a', 'blok_b', 'dapur', 'klinik']" :key="b" class="space-y-1.5">
                                    <Label class="capitalize text-[10px] sm:text-xs font-medium text-zinc-600">{{ b.replace('_', ' ') }}</Label>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 border-t border-zinc-100">
                                <div class="space-y-2">
                                    <Label class="text-[11px] sm:text-xs font-medium text-zinc-600">Dalam Lapas</Label>
                                    <Input type="number" v-model="form.dalam_lapas" class="h-12 sm:h-11 text-base sm:text-sm rounded-xl sm:rounded-lg" />
                                </div>
                                <div class="space-y-2">
                                    <Label class="text-[11px] sm:text-xs font-medium text-zinc-600">Luar Lapas</Label>
                            ...
                            <div v-for="(items, key) in { rawat_inap_items: 'Rawat Inap RS', berobat_items: 'Berobat RS', bon_luar_items: 'Lain-lain (Bon Luar)' }" :key="key" class="space-y-3 pt-2">
                                <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center bg-zinc-50 px-3 py-2.5 sm:py-2 rounded-xl sm:rounded-lg border border-zinc-100 gap-2">
                                    <Label class="text-[11px] font-bold text-zinc-700 uppercase">{{ items }}</Label>
                ...
                <div v-if="isPreviewActive" class="animate-in fade-in zoom-in-95 duration-300 font-sans w-full">
                    ...
                    <Card class="border-none shadow-xl bg-white p-6 sm:p-8 md:p-12 rounded-2xl sm:rounded-xl text-zinc-900 font-sans text-[13px] sm:text-[14px] md:text-[15px] leading-relaxed mb-6 overflow-x-auto custom-scrollbar">
                     ...
                     <div class="sticky bottom-4 sm:bottom-6 flex flex-col sm:flex-row gap-3 bg-zinc-900/90 backdrop-blur-md p-4 sm:p-4 rounded-3xl border border-zinc-800 shadow-xl max-w-4xl mx-auto z-20">
                         ...
                         <Button @click="submitLaporan" type="button" class="w-full sm:w-2/3 h-14 sm:h-14 bg-amber-500 hover:bg-amber-600 text-white rounded-2xl font-bold text-sm sm:text-base shadow-lg transition-all active:scale-[0.99] border-0" :disabled="form.processing">
                            ...
                            <span v-else class="flex items-center">
                                <Check class="w-5 h-5 mr-2"/> SIMPAN PERUBAHAN
                            </span>
                         </Button>
                     </div>
                </div>
